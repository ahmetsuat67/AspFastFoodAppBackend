    ...
    login: async (req, res) => {
        ...
        */
        ...
        if (username && password) {
            ...
            const user = await User.findOne({ username, password }) // bu username ve passwordde eşleşen bir kullanıcı var mı diye bakıyoruz 

            if (user) { // eğer varsa

                if (user.isActive) { // ve eğer kullanıcı aktifse
                    ...
                    res.send({ // eğer kullanıcı aktifse token oluşturup gönderiyoruz
                        error: false,
                        token: setToken(user) // setToken fonksiyonu ile token oluşturuyoruz
                    })

                } else { // eğer kullanıcı aktif değilse
        ...
        } else { // eğer kullanıcı adı veya şifre girilmemişse
            ...
            jwt.verify(refreshToken, process.env.REFRESH_KEY, async function (err, userData) {
                ...
                if (err) {
                    ...
                    if (_id && password) {
                        ...
                        const user = await User.findOne({ _id }) // findOne şifreleme yapan settokenı çalıştırır
